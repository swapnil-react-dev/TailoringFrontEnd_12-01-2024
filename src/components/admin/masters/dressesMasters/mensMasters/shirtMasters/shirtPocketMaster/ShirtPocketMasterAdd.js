...
import { addShirtPocket,editDeleteShirtPocket,UploadShirtPocketImage } from "../../../../../../../actions/actions";


const ShirtPocketMasterAdd = () => {
    ...
    const editid = editData ? editData.shirt_pocket_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const shirt_pocket_image = useSelector(state => state.shirt_pocket.shirt_pocket_image);
    console.log("shirtpocket", shirt_pocket_image)

    const [data, setData] = useState({
        shirt_pocket_name: editData ? editData.shirt_pocket_name : '',
        shirt_pocket_image: editData ? editData.shirt_pocket_image : '',
        shirt_pocket_charges: editData ? editData.shirt_pocket_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadShirtPocketImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            shirt_pocket_id: editid,
            shirt_pocket_image : shirt_pocket_image == '' ? editData ? editData.shirt_pocket_image : shirt_pocket_image : shirt_pocket_image, 
        ...
        if (editData) {
            dispatch(editDeleteShirtPocket(roleData));
        } else {
            dispatch(addShirtPocket(roleData));
        }
    }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Pocket Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="shirt_pocket_name"
                                            value={(data.shirt_pocket_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Pocket Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="text"
                                            onChange={handleChange}
                                            name="shirt_pocket_charges"
                                            value={(data.shirt_pocket_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Pocket Image</Typography>
                                    </Grid>
...
export default ShirtPocketMasterAdd;