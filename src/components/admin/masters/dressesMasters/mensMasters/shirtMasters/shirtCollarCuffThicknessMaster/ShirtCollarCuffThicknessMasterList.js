...
import { editDeleteShirtCollarCuffThickness, getShirtCollarCuffThicknessList } from "../../../../../../../actions/actions";
...
const ShirtCollarCuffThicknessMasterList = () => {
    ...
    const [searchParam] = useState(["shirt_collar_cuff_thickness_name","shirt_collar_cuff_thickness_image","shirt_collar_cuff_thickness_charges"]);


    const fields = ['id', "user_role",];
    const labels = ['Id', "user_role",];

    const [isDelete, setIsDelete] = useState(false);
    const [deleteInfo, setDeleteInfo] = useState();

    const rows = useSelector(state => state.shirt_collar_cuff_thickness.shirt_collar_cuff_thickness)
    

    const fetchData = () => {
        dispatch(getShirtCollarCuffThicknessList())
    };
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteShirtCollarCuffThickness({ shirt_collar_cuff_thickness_id : id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                <Breadcrumb
                    routeSegments={[
                        ...
                        { name: 'Shirt Collar Cuff Thickness List' },
                    ]}
                />
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        <Grid item md={7} sm={6} xs={12}>
                            <Box sx={{ display: 'flex' }}>
                                ...
                                ShirtCollarCuffThicknessMasterList
                                </Typography>
                            </Box>
                        </Grid>
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <NavLink to={{ pathname: `/admin/add-shirt-collar-cuff-thickness` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            ...
                            <StyledTableCell align="center" className={`${styles.table_head}`}>Action</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Shirt Collar Cuff Thickness Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Shirt Collar Cuff Thickness Charges</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Shirt Collar Cuff Thickness Image</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-shirt-collar-cuff-thickness` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.shirt_collar_cuff_thickness_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.shirt_collar_cuff_thickness_charges}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>
                                            <img src={row.shirt_collar_cuff_thickness_image} style={{width:'60px', height: '50px'}}/>
                                            </StyledTableCell>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.shirt_collar_cuff_thickness_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.shirt_collar_cuff_thickness_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
...
export default ShirtCollarCuffThicknessMasterList;