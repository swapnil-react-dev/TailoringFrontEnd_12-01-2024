...
import { editDeleteFabrics, getFabricsList } from "../../../../../actions/actions";
...
const FabricsList = () => {
    ...
    const [searchParam] = useState(["fabric_code", "fabric_name", "brand_id", "pattern_id", "fabric_quality_id", "compositions", "gsm", "thickness", "colors", "color_accuracy", "fabric_feel_id", "weight", "epi", "ppi", "width", "category", "fabric_use", "occasion_id", "mfr_id", "purchase_unit", "sell_unit", "moq", "made_in"]);
    ...
    const rows = useSelector(state => state.fabrics.fabrics)
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteFabrics({ fabric_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <NavLink to={{ pathname: `/admin/add-fabrics` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Fabric Code</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Fabric Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Brand Id</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Pattern Id</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Fabric Quality Id</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Compositions</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-fabrics` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.fabric_code}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`} sx={{ color: 'blue', cursor: "pointer" }} onClick={() => handleDetails(row)}>
                                            {row.fabric_name}
                                        </StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.brand_id}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.pattern_id}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.fabric_quality_id}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.compositions}</StyledTableCell>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.fabric_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.fabric_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
                        {openOption === 'Details' ? 'View Fabrics Details' : ``}
                    </BootstrapDialogTitle>
                    <DialogContent dividers >
                        {openOption === 'Details' &&
                            <FabricsDetails detailInfo={info} />
                        }
                    </DialogContent>
                </BootstrapDialog>
            </Paper>
