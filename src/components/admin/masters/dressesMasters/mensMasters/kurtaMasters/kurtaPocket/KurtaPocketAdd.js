...
import { addKurtaPocket, editDeleteKurtaPocket, UploadKurtaPocketImage } from "../../../../../../../actions/actions";

const KurtaPocketAdd = () => {
    ...
    const editid = editData ? editData.kurta_pocket_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const kurta_pocket_image = useSelector(state => state.kurta_pocket.kurta_pocket_image)

    const [data, setData] = useState({
        kurta_pocket_name: editData ? editData.kurta_pocket_name : '',
        kurta_pocket_image: editData ? editData.kurta_pocket_image : '',
        kurta_pocket_charges: editData ? editData.kurta_pocket_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadKurtaPocketImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            kurta_pocket_id: editid,
            kurta_pocket_image: kurta_pocket_image == '' ? editData ? editData.kurta_pocket_image : kurta_pocket_image : kurta_pocket_image,
        ...
        if (editData) {
            dispatch(editDeleteKurtaPocket(roleData));
        } else {
            dispatch(addKurtaPocket(roleData));
        }
    }
    return ( 
        <>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Pocket Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="kurta_pocket_name"
                                            value={(data.kurta_pocket_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Pocket Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="kurta_pocket_charges"
                                            value={(data.kurta_pocket_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Pocket Image</Typography>
                                    </Grid>
