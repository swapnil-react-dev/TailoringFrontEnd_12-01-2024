...
import { editDeleteSherwaniCollarDesign, getSherwaniCollarDesignList } from "../../../../../../../actions/actions";
...
const SherwaniCollarDesignList = () => {
    ...
    const [searchParam] = useState(["sherwani_collar_design_name", "sherwani_collar_design_image", "sherwani_collar_design_charges"]);
    ...
    const rows = useSelector(state => state.sherwani_collar_design.sherwani_collar_design)
    console.log("rows", rows)

    const fetchData = () => {
        dispatch(getSherwaniCollarDesignList())
    };
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteSherwaniCollarDesign({ sherwani_collar_design_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                <Breadcrumb
                    routeSegments={[
                        ...
                        { name: 'Sherwani Collar Design List' },
                    ]}
                />
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        <Grid item md={7} sm={6} xs={12}>
                            <Box sx={{ display: 'flex' }}>
                                <Typography className="form-title" >
                                    Sherwani Collar Design Master List
                                </Typography>
                            </Box>
                        </Grid>
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            />
                            <NavLink to={{ pathname: `/admin/add-sherwani-collar-design` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Sherwani Collar Design Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Sherwani Collar Design Charges</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Sherwani Collar Design Image</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-sherwani-collar-design` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.sherwani_collar_design_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.sherwani_collar_design_charges}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>
                                            <img src={row.sherwani_collar_design_image} style={{ width: "60px", height: "50px" }} />
                                        </StyledTableCell>
                                    </StyledTableRow>
                                ))}
                        </TableBody>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.sherwani_collar_design_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.sherwani_collar_design_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
...
export default SherwaniCollarDesignList;