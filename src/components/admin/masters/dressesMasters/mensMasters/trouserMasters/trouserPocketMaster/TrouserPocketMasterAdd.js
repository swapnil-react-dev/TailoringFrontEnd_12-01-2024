...
import { addTrouserPocket,editDeleteTrouserPocket, UploadTrouserPocketImage } from "../../../../../../../actions/actions";


const TrouserPocketMasterAdd = () => {
    ...
    const editid = editData ? editData.trouser_back_pocket_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);

    const [data, setData] = useState({
        trouser_back_pocket_name: editData ? editData.trouser_back_pocket_name : '',
        trouser_back_pocket_charges: editData ? editData.trouser_back_pocket_charges : '',
        // trouser_back_pocket_image: editData ? editData.trouser_back_pocket_image : '',
    });
    const [previewImage, setPreviewImage] = useState("");
    const trouser_back_pocket_image = useSelector(state => state.trouser_pocket.trouser_pocket_image)
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadTrouserPocketImage(formData));
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            trouser_back_pocket_id : editid,
            trouser_back_pocket_image : trouser_back_pocket_image == '' ? editData ? editData.trouser_back_pocket_image : trouser_back_pocket_image : trouser_back_pocket_image,
        ...
        if (editData) {
            dispatch(editDeleteTrouserPocket(roleData));
        } else {
            dispatch(addTrouserPocket(roleData));
        }
    }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Back Pocket Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="trouser_back_pocket_name"
                                            value={(data.trouser_back_pocket_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Back Pocket Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="trouser_back_pocket_charges"
                                            value={(data.trouser_back_pocket_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Back Pocket Image</Typography>
                                    </Grid>
                                    <Grid item xs={4} className={`${styles.image_input}`} >
                                        <TLRinput
                                            ...
                                            size="small"
                                            fullWidth
                                            name="trouser_back_pocket_image"
                                            accept="image/*"
                                            type="file"
                                            onChange={uploadFile}
                                            InputProps={{
...
export default TrouserPocketMasterAdd;