...
import { addShirtBottom,editDeleteShirtBottom , UploadShirtBottomImage} from "../../../../../../../actions/actions";


const ShirtBottomAdd = () => {
    ...
    const editid = editData ? editData.shirt_bottom_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);

    const [data, setData] = useState({
        shirt_bottom_name : editData ? editData.shirt_bottom_name : '',
        // shirt_bottom_image : editData ? editData.shirt_bottom_image : '',
        shirt_bottom_charges : editData ? editData.shirt_bottom_charges : ''                      
    });
    const shirt_bottom_image = useSelector(state => state.shirt_bottom.shirt_bottom_image)
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadShirtBottomImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            shirt_bottom_id: editid,
            shirt_bottom_image : shirt_bottom_image == '' ? editData ? editData.shirt_bottom_image : shirt_bottom_image : shirt_bottom_image,
        ...
        if (editData) {
            dispatch(editDeleteShirtBottom(roleData));
        } else {
            dispatch(addShirtBottom(roleData));
        }
    }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Bottom Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="shirt_bottom_name"
                                            value={(data.shirt_bottom_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Bottom Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="text"
                                            onChange={handleChange}
                                            name="shirt_bottom_charges"
                                            value={(data.shirt_bottom_charges)}
                                        />
                                    </Grid>

                                    {/* <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Bottom Image</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            required
                                            ...
                                            type="text"
                                            onChange={handleChange}
                                            name="shirt_bottom_image"
                                            value={(data.shirt_bottom_image)}
                                        />
                                    </Grid> */}
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Bottom Image</Typography>
