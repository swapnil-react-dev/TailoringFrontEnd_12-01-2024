...
import { editDeleteShirtFront, getShirtFrontList } from "../../../../../../../actions/actions";
...
const ShirtFrontMasterList = () => {
    ...
    const [searchParam] = useState(["shirt_front_name","shirt_front_image","shirt_front_charges"]);
    ...
    const rows = useSelector(state => state.shirt_front.shirt_front)

    const fetchData = () => {
        dispatch(getShirtFrontList())
    };
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteShirtFront({ shirt_front_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <NavLink to={{ pathname: `/admin/add-shirt-front` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`}>Shirt Front Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`}>Shirt Front Charges</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`}>Shirt Front Image</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-shirt-front` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.shirt_front_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.shirt_front_charges}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>
                                         <img src={row.shirt_front_image} style={{ width:"60px" , height:"50px"}}/>
                                        </StyledTableCell>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.shirt_front_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.shirt_front_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
...
export default ShirtFrontMasterList;