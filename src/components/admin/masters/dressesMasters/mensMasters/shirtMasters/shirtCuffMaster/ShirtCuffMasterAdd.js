...
import { addShirtCuff,editDeleteShirtCuff } from "../../../../../../../actions/actions";


const ShirtCuffMasterAdd = () => {
    ...
    const editid = editData ? editData.shirt_cuff_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);

    const [data, setData] = useState({
        shirt_cuff_name: editData ? editData.shirt_cuff_name : '',
        shirt_cuff_image: editData ? editData.shirt_cuff_image : '',
        shirt_cuff_charges: editData ? editData.shirt_cuff_charges : '',
    });
    ...
    const onSubmit = (event) => {
        ...
        if (editData) {
            dispatch(editDeleteShirtCuff(roleData));
        } else {
            dispatch(addShirtCuff(roleData));
        }
    }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Cuff Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="shirt_cuff_name"
                                            value={(data.shirt_cuff_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Cuff Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="text"
                                            onChange={handleChange}
                                            name="shirt_cuff_charges"
                                            value={(data.shirt_cuff_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shirt Cuff Image</Typography>
                                    </Grid>
...
export default ShirtCuffMasterAdd;