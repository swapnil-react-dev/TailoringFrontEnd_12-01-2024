...
import { editDeleteKurtaSleeve, getKurtaSleeveList } from "../../../../../../../actions/actions";
...
const KurtaSleeveList = () => {
    ...
    const [searchParam] = useState(["kurta_sleeve_name", "kurta_sleeve_image", "kurta_sleeve_charges"]);
    ...
    const rows = useSelector(state => state.kurta_sleeve.kurta_sleeve)

    const fetchData = () => {
        dispatch(getKurtaSleeveList())
    };
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteKurtaSleeve({ kurta_sleeve_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            />
                            <NavLink to={{ pathname: `/admin/add-kurta-sleeve` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Sleeve Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Sleeve Charges</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Sleeve Image</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-kurta-sleeve` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.kurta_sleeve_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.kurta_sleeve_charges}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>
                                            <img src={row.kurta_sleeve_image} style={{ width: "60px", height: "50px" }} />
                                        </StyledTableCell>
                                    </StyledTableRow>
                                ))}
                        </TableBody>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.kurta_sleeve_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.kurta_sleeve_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
...
export default KurtaSleeveList;