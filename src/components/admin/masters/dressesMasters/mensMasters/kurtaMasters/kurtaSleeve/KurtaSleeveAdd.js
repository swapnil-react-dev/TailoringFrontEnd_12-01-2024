...
import { addKurtaSleeve, editDeleteKurtaSleeve, UploadKurtaSleeveImage } from "../../../../../../../actions/actions";

const KurtaSleeveAdd = () => {
    ...
    const editid = editData ? editData.kurta_sleeve_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const kurta_sleeve_image = useSelector(state => state.kurta_sleeve.kurta_sleeve_image)

    const [data, setData] = useState({
        kurta_sleeve_name: editData ? editData.kurta_sleeve_name : '',
        kurta_sleeve_image: editData ? editData.kurta_sleeve_image : '',
        kurta_sleeve_charges: editData ? editData.kurta_sleeve_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadKurtaSleeveImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            kurta_sleeve_id: editid,
            kurta_sleeve_image: kurta_sleeve_image == '' ? editData ? editData.kurta_sleeve_image : kurta_sleeve_image : kurta_sleeve_image,
        ...
        if (editData) {
            dispatch(editDeleteKurtaSleeve(roleData));
        } else {
            dispatch(addKurtaSleeve(roleData));
        }
    }
    return ( 
        <>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Sleeve Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="kurta_sleeve_name"
                                            value={(data.kurta_sleeve_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Sleeve Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="kurta_sleeve_charges"
                                            value={(data.kurta_sleeve_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Sleeve Image</Typography>
                                    </Grid>
