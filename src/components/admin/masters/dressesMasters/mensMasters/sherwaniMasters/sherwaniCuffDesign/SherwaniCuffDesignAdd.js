...
import { addSherwaniCuffDesign, editDeleteSherwaniCuffDesign, UploadSherwaniCuffDesignImage } from "../../../../../../../actions/actions";

const SherwaniCuffDesignAdd = () => {
    ...
    const editid = editData ? editData.sherwani_cuff_design_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_cuff_design_image = useSelector(state => state.sherwani_cuff_design.sherwani_cuff_design_image)

    const [data, setData] = useState({
        sherwani_cuff_design_name: editData ? editData.sherwani_cuff_design_name : '',
        sherwani_cuff_design_image: editData ? editData.sherwani_cuff_design_image : '',
        sherwani_cuff_design_charges: editData ? editData.sherwani_cuff_design_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniCuffDesignImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_cuff_design_id: editid,
            sherwani_cuff_design_image: sherwani_cuff_design_image == '' ? editData ? editData.sherwani_cuff_design_image : sherwani_cuff_design_image : sherwani_cuff_design_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniCuffDesign(roleData));
        } else {
            dispatch(addSherwaniCuffDesign(roleData));
        }
                        ...
                        <Grid item md={7} sm={6} xs={12}>
                            <Box sx={{ display: 'flex' }}>
                                <Typography className="form-title">
                                    Create Sherwani Cuff Design Add
                                </Typography>
                            </Box>
                        </Grid>
                    </Grid>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Design Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_cuff_design_name"
                                            value={(data.sherwani_cuff_design_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Design Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_cuff_design_charges"
                                            value={(data.sherwani_cuff_design_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Design Image</Typography>
                                    </Grid>
                                    ...
                                    <Grid item xs={4} sx={{ border: "1px solid black", borderRight: "none", height: "38%", marginLeft: "9px" }}>
                                        <TLRinput
                                            ...
                                            size="small"
                                            fullWidth
                                            name="sherwani_cuff_design_image"
...
export default SherwaniCuffDesignAdd;