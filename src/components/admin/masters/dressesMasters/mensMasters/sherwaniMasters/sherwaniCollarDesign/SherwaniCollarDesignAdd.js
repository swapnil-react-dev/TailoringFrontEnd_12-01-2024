...
import { addSherwaniCollarDesign, editDeleteSherwaniCollarDesign, UploadSherwaniCollarDesignImage } from "../../../../../../../actions/actions";
const SherwaniCollarDesignAdd = () => {
    ...
    const editid = editData ? editData.sherwani_collar_design_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_collar_design_image = useSelector(state => state.sherwani_collar_design.sherwani_collar_design_image)

    const [data, setData] = useState({
        sherwani_collar_design_name: editData ? editData.sherwani_collar_design_name : '',
        sherwani_collar_design_image: editData ? editData.sherwani_collar_design_image : '',
        sherwani_collar_design_charges: editData ? editData.sherwani_collar_design_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniCollarDesignImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_collar_design_id: editid,
            sherwani_collar_design_image: sherwani_collar_design_image == '' ? editData ? editData.sherwani_collar_design_image : sherwani_collar_design_image : sherwani_collar_design_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniCollarDesign(roleData));
        } else {
            dispatch(addSherwaniCollarDesign(roleData));
        }
                        ...
                        <Grid item md={7} sm={6} xs={12}>
                            <Box sx={{ display: 'flex' }}>
                                <Typography className="form-title">
                                    Create Sherwani Collar Design Add
                                </Typography>
                            </Box>
                        </Grid>
                    </Grid>
                                ...
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    ...
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_collar_design_name"
                                            value={(data.sherwani_collar_design_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                    ...
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_collar_design_charges"
                                            value={(data.sherwani_collar_design_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                    ...
                                    <Grid item xs={4} sx={{ border: "1px solid black", borderRight: "none", height: "38%", marginLeft: "9px" }}>
                                        <TLRinput
                                            ...
                                            size="small"
                                            fullWidth
                                            name="sherwani_collar_design_image"
...
export default SherwaniCollarDesignAdd;