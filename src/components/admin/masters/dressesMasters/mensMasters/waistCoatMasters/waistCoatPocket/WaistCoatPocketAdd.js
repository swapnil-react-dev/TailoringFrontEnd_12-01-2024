...
import { addWaistCoatPocket, editDeleteWaistCoatPocket, UploadWaistCoatPocketImage } from "../../../../../../../actions/actions";
const WaistCoatPocketAdd = () => {
    ...
    const editid = editData ? editData.waist_coat_pocket_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);

    const waist_coat_pocket_image = useSelector(state => state.waist_coat_pocket.waist_coat_pocket_image)
    console.log("image", waist_coat_pocket_image)

    const [data, setData] = useState({
        waist_coat_pocket_name: editData ? editData.waist_coat_pocket_name : '',
        waist_coat_pocket_image: editData ? editData.waist_coat_pocket_image : '',
        waist_coat_pocket_charges: editData ? editData.waist_coat_pocket_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadWaistCoatPocketImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            waist_coat_pocket_id: editid,
            waist_coat_pocket_image: waist_coat_pocket_image == '' ? editData ? editData.waist_coat_pocket_image : waist_coat_pocket_image : waist_coat_pocket_image,
        ...
        if (editData) {
            dispatch(editDeleteWaistCoatPocket(roleData));
        } else {
            dispatch(addWaistCoatPocket(roleData));
        }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Pocket Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="waist_coat_pocket_name"
                                            value={(data.waist_coat_pocket_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Pocket Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="waist_coat_pocket_charges"
                                            value={(data.waist_coat_pocket_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Pocket Image</Typography>
                                    </Grid>
...
export default WaistCoatPocketAdd;