...
import { editDeleteStitchingCenters, getStitchingCentersList } from "../../../../../actions/actions";
...
const StitchingCentersList = () => {
    ...
    const [searchParam] = useState(["stitching_center_code", "stitching_center_name", "stitching_center_email_id", "stitching_center_address", "stitching_center_city", "stitching_center_pincode", "stitching_center_contact_no"]);
    ...
    const rows = useSelector(state => state.stitching_centers.stitching_centers)
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteStitchingCenters({ mfr_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <NavLink to={{ pathname: `/admin/add-stitching` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Code</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Email Id</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Address</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center City</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Pincode</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Stitching Center Contact No</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-stitching` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_code}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`} sx={{ color: 'blue', cursor: "pointer" }} onClick={() => handleDetails(row)}>
                                            {row.stitching_center_name}
                                        </StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_email_id}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_address}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_city}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_pincode}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.stitching_center_contact_no}</StyledTableCell>
                ...
                >
                    <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
                        {openOption === 'Details' ? 'View Stitching Centers Details' : ``}
                    </BootstrapDialogTitle>
                    <DialogContent dividers >
                        {openOption === 'Details' &&
                            <StitchingCentersDetails detailInfo={info} />
                        }
                    </DialogContent>
                </BootstrapDialog>
            </Paper>
...
export default StitchingCentersList;