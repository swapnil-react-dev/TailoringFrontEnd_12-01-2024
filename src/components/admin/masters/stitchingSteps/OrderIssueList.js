...
import {editDeleteOrderIssue,getOrderIssueList} from "../../../../actions/actions"
...
const OrderIssueList = () => {
    ...
    const [searchParam] = useState(["issue_no", "issue_date", "item_type", "fabric_id", "fabric_code", "item_id", "quantity"]);
    ...
    const rows = useSelector(state => state.order_issue.order_issue)
    console.log("rows123", rows)

    const fetchData = () => {
        dispatch(getOrderIssueList())
    };
    useEffect(() => {
        fetchData()
    }, []);
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Issue No</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Issue Date</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Fabric</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Item Type</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Item Code</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Quantity</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-order-issue` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.issue_no}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.issue_date}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.item_type}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.fabric_name} ({row.fabric_code})</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.item_code}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.quantity}</StyledTableCell>
                                    </StyledTableRow>
                                ))}
                        </TableBody>
                    </Table>
