...
import { editDeleteKurtaStyle, getKurtaStyleList} from "../../../../../../../actions/actions";
...
const KurtaStyleList = () => {
    ...
    const [searchParam] = useState(["kurta_style_name", "kurta_style_image", "kurta_style_charges"]);
    ...
    const rows = useSelector(state => state.kurta_style.kurta_style)

    const fetchData = () => {
        dispatch(getKurtaStyleList())
    };
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteKurtaStyle({ kurta_style_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
            ...
            <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            />
                            <NavLink to={{ pathname: `/admin/add-kurta-style` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Style Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Style Charges</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Kurta Style Image</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-kurta-style` }} state={row} className={`${styles.underline}`}>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.kurta_style_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.kurta_style_charges}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>
                                            <img src={row.kurta_style_image} style={{ width: "60px", height: "50px" }} />
                                        </StyledTableCell>
                                    </StyledTableRow>
                                ))}
                        </TableBody>
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.kurta_style_name}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.kurta_style_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
