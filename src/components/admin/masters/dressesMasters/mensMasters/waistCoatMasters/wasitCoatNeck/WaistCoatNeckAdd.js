...
import { Box, Button, Grid, MenuItem, Paper, Snackbar, Typography, Avatar, FormControlLabel, Checkbox, FormGroup } from "@mui/material";
...
import { addWaistCoatNeck, editDeleteWaistCoatNeck, UploadWaistCoatNeckImage } from "../../../../../../../actions/actions";
const WaistCoatNeckAdd = () => {
    ...
    const editid = editData ? editData.waist_coat_neck_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const waist_coat_neck_image = useSelector(state => state.waist_coat_neck.waist_coat_neck_image)

    const [data, setData] = useState({
        waist_coat_neck_name: editData ? editData.waist_coat_neck_name : '',
        waist_coat_neck_image: editData ? editData.waist_coat_neck_image : '',
        waist_coat_neck_charges: editData ? editData.waist_coat_neck_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadWaistCoatNeckImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            waist_coat_neck_id: editid,
            waist_coat_neck_image: waist_coat_neck_image == '' ? editData ? editData.waist_coat_neck_image : waist_coat_neck_image : waist_coat_neck_image,
        ...
        if (editData) {
            dispatch(editDeleteWaistCoatNeck(roleData));
        } else {
            dispatch(addWaistCoatNeck(roleData));
        }
    }
    return (
        <>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Neck Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="waist_coat_neck_name"
                                            value={(data.waist_coat_neck_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Neck Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="waist_coat_neck_charges"
                                            value={(data.waist_coat_neck_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Neck Image</Typography>
                                    </Grid>
...
export default WaistCoatNeckAdd;