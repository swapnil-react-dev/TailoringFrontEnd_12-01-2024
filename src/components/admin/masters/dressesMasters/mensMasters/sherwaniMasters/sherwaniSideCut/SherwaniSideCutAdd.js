...
import { addSherwaniSideCut, editDeleteSherwaniSideCut, UploadSherwaniSideCutImage } from "../../../../../../../actions/actions";

const SherwaniSideCutAdd = () => {
    ...
    const editid = editData ? editData.sherwani_side_cut_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_side_cut_image = useSelector(state => state.sherwani_side_cut.sherwani_side_cut_image)

    const [data, setData] = useState({
        sherwani_side_cut_name: editData ? editData.sherwani_side_cut_name : '',
        sherwani_side_cut_charges: editData ? editData.sherwani_side_cut_charges : '',
        // sherwani_side_cut_image: editData ? editData.sherwani_side_cut_image : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniSideCutImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_side_cut_id: editid,
            sherwani_side_cut_image: sherwani_side_cut_image == '' ? editData ? editData.sherwani_side_cut_image : sherwani_side_cut_image : sherwani_side_cut_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniSideCut(roleData));
        } else {
            dispatch(addSherwaniSideCut(roleData));
        }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Side Cut Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_side_cut_name"
                                            value={(data.sherwani_side_cut_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Side Cut Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_side_cut_charges"
                                            value={(data.sherwani_side_cut_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Side Cut Image</Typography>
                                    </Grid>
...
export default SherwaniSideCutAdd;