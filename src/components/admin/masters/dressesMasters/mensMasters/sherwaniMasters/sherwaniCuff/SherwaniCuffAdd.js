...
import { addSherwaniCuff, editDeleteSherwaniCuff, UploadSherwaniCuffImage } from "../../../../../../../actions/actions";

const SherwaniCuffAdd = () => {
    ...
    const editid = editData ? editData.sherwani_cuff_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_cuff_image = useSelector(state => state.sherwani_cuff.sherwani_cuff_image)

    const [data, setData] = useState({
        sherwani_cuff_name: editData ? editData.sherwani_cuff_name : '',
        sherwani_cuff_image: editData ? editData.sherwani_cuff_image : '',
        sherwani_cuff_charges: editData ? editData.sherwani_cuff_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniCuffImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_cuff_id: editid,
            sherwani_cuff_image: sherwani_cuff_image == '' ? editData ? editData.sherwani_cuff_image : sherwani_cuff_image : sherwani_cuff_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniCuff(roleData));
        } else {
            dispatch(addSherwaniCuff(roleData));
        }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_cuff_name"
                                            value={(data.sherwani_cuff_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_cuff_charges"
                                            value={(data.sherwani_cuff_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Cuff Image</Typography>
                                    </Grid>
...
export default SherwaniCuffAdd;