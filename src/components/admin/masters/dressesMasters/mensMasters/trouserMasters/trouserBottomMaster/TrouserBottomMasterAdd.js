...
import { addTrouserBottom,editDeleteTrouserBottom , UploadTrouserBottomImage} from "../../../../../../../actions/actions";


const TrouserBottomMasterAdd = () => {
    ...
    const editid = editData ? editData.trouser_bottom_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);

    const [data, setData] = useState({
        trouser_bottom_name: editData ? editData.trouser_bottom_name : '',
        trouser_bottom_charges: editData ? editData.trouser_bottom_charges : '',
        // trouser_bottom_image: editData ? editData.trouser_bottom_image : '',
    });
    const [previewImage, setPreviewImage] = useState("");
    const trouser_bottom_image = useSelector(state => state.trouser_bottom.trouser_bottom_image)
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadTrouserBottomImage(formData));
            ...
            reader.addEventListener(
              ...
              if (selectedFile) {
                ...
                setPreviewImage ({...previewImage,  trouser_bottom_image: selectedFile});
              }
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            trouser_bottom_id: editid,
            trouser_bottom_image : trouser_bottom_image == '' ? editData ? editData.trouser_bottom_image : trouser_bottom_image : trouser_bottom_image,
        ...
        if (editData) {
            dispatch(editDeleteTrouserBottom(roleData));
        } else {
            dispatch(addTrouserBottom(roleData));
        }
    }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Bottom Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="trouser_bottom_name"
                                            value={(data.trouser_bottom_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Bottom Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="trouser_bottom_charges"
                                            value={(data.trouser_bottom_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Trouser Bottom Image</Typography>
                                    </Grid>
...
export default TrouserBottomMasterAdd;