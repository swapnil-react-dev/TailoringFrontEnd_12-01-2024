...
import { addSherwaniPocket, editDeleteSherwaniPocket, UploadSherwaniPocketImage } from "../../../../../../../actions/actions";

const SherwaniPocketAdd = () => {
    ...
    const editid = editData ? editData.sherwani_pocket_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_pocket_image = useSelector(state => state.sherwani_pocket.sherwani_pocket_image)

    const [data, setData] = useState({
        sherwani_pocket_name: editData ? editData.sherwani_pocket_name : '',
        sherwani_pocket_charges: editData ? editData.sherwani_pocket_charges : '',
        sherwani_pocket_image: editData ? editData.sherwani_pocket_image : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniPocketImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_pocket_id: editid,
            sherwani_pocket_image: sherwani_pocket_image == '' ? editData ? editData.sherwani_pocket_image : sherwani_pocket_image : sherwani_pocket_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniPocket(roleData));
        } else {
            dispatch(addSherwaniPocket(roleData));
        }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Pocket Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_pocket_name"
                                            value={(data.sherwani_pocket_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Pocket Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_pocket_charges"
                                            value={(data.sherwani_pocket_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Pocket Image</Typography>
                                    </Grid>
...
export default SherwaniPocketAdd;