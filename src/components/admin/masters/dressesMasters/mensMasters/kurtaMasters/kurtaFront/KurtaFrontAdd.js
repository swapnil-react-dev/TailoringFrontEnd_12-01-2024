...
import { addKurtaFront, editDeleteKurtaFront, UploadKurtaFrontImage } from "../../../../../../../actions/actions";
...
const KurtaFrontAdd = () => {
    ...
    const editid = editData ? editData.kurta_front_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const kurta_front_image = useSelector(state => state.kurta_front.kurta_front_image)

    const [data, setData] = useState({
        kurta_front_name: editData ? editData.kurta_front_name : '',
        kurta_front_image: editData ? editData.kurta_front_image : '',
        kurta_front_charges: editData ? editData.kurta_front_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadKurtaFrontImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            kurta_front_id: editid,
            kurta_front_image: kurta_front_image == '' ? editData ? editData.kurta_front_image : kurta_front_image : kurta_front_image,
        ...
        if (editData) {
            dispatch(editDeleteKurtaFront(roleData));
        } else {
            dispatch(addKurtaFront(roleData));
        }
    }
    return ( 
        <>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Front Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="kurta_front_name"
                                            value={(data.kurta_front_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Front Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="kurta_front_charges"
                                            value={(data.kurta_front_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Kurta Front Image</Typography>
                                    </Grid>
