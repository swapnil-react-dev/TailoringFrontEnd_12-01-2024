...
import { addSherwaniFront, editDeleteSherwaniFront, UploadSherwaniFrontImage } from "../../../../../../../actions/actions";

const SherwaniFrontAdd = () => {
    ...
    const editid = editData ? editData.sherwani_front_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const sherwani_front_image = useSelector(state => state.sherwani_front.sherwani_front_image)

    const [data, setData] = useState({
        sherwani_front_name: editData ? editData.sherwani_front_name : '',
        sherwani_front_image: editData ? editData.sherwani_front_image : '',
        sherwani_front_charges: editData ? editData.sherwani_front_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadSherwaniFrontImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            sherwani_front_id: editid,
            sherwani_front_image: sherwani_front_image == '' ? editData ? editData.sherwani_front_image : sherwani_front_image : sherwani_front_image,
        ...
        if (editData) {
            dispatch(editDeleteSherwaniFront(roleData));
        } else {
            dispatch(addSherwaniFront(roleData));
        }
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Front Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="sherwani_front_name"
                                            value={(data.sherwani_front_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Front Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="sherwani_front_charges"
                                            value={(data.sherwani_front_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Shewani Front Image</Typography>
                                    </Grid>
...
export default SherwaniFrontAdd;