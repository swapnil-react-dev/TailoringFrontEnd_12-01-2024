...
import { addWaistCoatType, editDeleteWaistCoatType, UploadWaistCoatTypeImage } from "../../../../../../../actions/actions";

const WaistCoatTypeAdd = () => {
    ...
    const editid = editData ? editData.waist_coat_type_id : null;
    let navigate = useNavigate();
    const [successMessage, setSuccessMessage] = useState(null);
    const waist_coat_type_image = useSelector(state => state.waist_coat_type.waist_coat_type_image)

    const [data, setData] = useState({
        waist_coat_type_name: editData ? editData.waist_coat_type_name : '',
        waist_coat_type_image: editData ? editData.waist_coat_type_image : '',
        waist_coat_type_charges: editData ? editData.waist_coat_type_charges : '',
    });
    ...
    const uploadFile = async (event) => {
        try {
            ...
            await dispatch(UploadWaistCoatTypeImage(formData));
        } catch (error) {
    ...
    const onSubmit = (event) => {
        const roleData = {
            ...data,
            waist_coat_type_id: editid,
            waist_coat_type_image: waist_coat_type_image == '' ? editData ? editData.waist_coat_type_image : waist_coat_type_image : waist_coat_type_image,
        ...
        if (editData) {
            dispatch(editDeleteWaistCoatType(roleData));
        } else {
            dispatch(addWaistCoatType(roleData));
        }
    }
    return (
        <>
                            ...
                            <Grid item xs={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                                <Grid container spacing={1} sx={{ mt: 0 }}>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Type Name</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinput
                                            ...
                                            name="waist_coat_type_name"
                                            value={(data.waist_coat_type_name)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Type Charges</Typography>
                                    </Grid>
                                    <Grid item xs={9} className={`${styles.grid_input}`}>
                                        <TLRinputNo
                                            required
                                            ...
                                            type="number"
                                            onChange={handleChange}
                                            name="waist_coat_type_charges"
                                            value={(data.waist_coat_type_charges)}
                                        />
                                    </Grid>
                                    <Grid item xs={3} className={`${styles.grid_lable}`}>
                                        <Typography className={`${styles.erp_lable}`}>Waist Coat Type Image</Typography>
                                    </Grid>
...
export default WaistCoatTypeAdd;