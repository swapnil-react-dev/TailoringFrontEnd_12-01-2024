...
import {editDeleteOrderIssue,getOrdersList} from "../../../../actions/actions"
...
const OrdersList = () => {
    ...
    const [searchParam] = useState(["order_no","order_date","user_id","measurement_id","order_id","color_id","shirt_customization","trouser_customization","blazer_customization","kurta_customization", "sherwani_customization" , "total_amount", "pajama_customization", "waist_coat_customization", "subtotal", "gstPer", "cgst", "sgst", "igst", "total_amount", "approve_flag"]);

    const fields = ['order_id', "order_date", "shirt_customization", "total_amount"];
    const labels = ['Id', "c","order_date", "shirt_customization", "total_amount"];
    ...
    const deleteRecord = (id) => {
        dispatch(editDeleteOrderIssue({ issue_id: id, modifiedBy: userInfo.userId, modifiedAt: dateConversionOnEntryPage(new Date()), changedUpdatedValue: 'delete' }))
        setIsDelete(false);
        fetchData()
    }
         ...
         <Paper className={`${styles.main_container}`}>
                ...
                <Box sx={{ display: 'flex' }}>
                    <Grid container spacing={2} sx={{ pt: 1.5, pb: 1.5 }}>
                        ...
                        <Grid item md={5} sm={6} xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            />
                            <NavLink to={{ pathname: `/admin/add-order-issue` }}>
                    ...
                    <Table size="small" stickyHeader>
                        <TableHead>
                            <StyledTableRow>
                                ...
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Order No</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Order Date</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Fabric Name</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Order Id</StyledTableCell>
                                <StyledTableCell align="Left" className={`${styles.table_head}`} >Total Amount</StyledTableCell>
                                <StyledTableCell align="center" className={`${styles.table_head}`}>Issue</StyledTableCell>
                            ...
                            {rows && search(rows)
                                ...
                                .map((row, index) => (
                                    <StyledTableRow
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <Container className={`${styles.tbl_action}`}>
                                                                <>
                                                                    <NavLink to={{ pathname: `/admin/add-orders` }} state={row} className={`${styles.underline}`}>
                                                                    ...
                                                                    <Button className={`${styles.list_tbl_action_btn}`} onClick={() => {handleDelete(row); popupState.close() }}>
                                                                        <DeleteForeverOutlinedIcon /> Delete Record 
                                                                    </Button>
                                                                </>
                                                            </Container>
                                                        </Popover>
                                        ...
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}> {row.order_no}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{dateConversion(row.order_date)}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.fabric_name}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.order_id}</StyledTableCell>
                                        <StyledTableCell align="left" className={`${styles.list_table_body}`}>{row.total_amount}</StyledTableCell>
                                        <StyledTableCell align="center" className={`${styles.list_table_body}`}>
                                        <NavLink to={{ pathname: `/admin/add-order-issue`}} 
                                        state={row} 
                                        // details={row}
                                        >
                                        <Button sx={{fontSize:"10px", height:"18px" , background:"#336699", textDecoration: 'none' }} variant='contained' color="primary" type="submit" >Issue</Button>
                                        </NavLink>
                                        </StyledTableCell>
                                    </StyledTableRow>
                                ))}
                ...
                <Dialog open={isDelete}
                    ...
                    <DialogContent className={`${styles.dilog_delete_content}`}>
                        ...
                        Do you really want to delete these records ?  # {deleteInfo && deleteInfo.issue_no}
                    </DialogContent>
                    <DialogActions sx={{ padding: '10px', }}>
                        <Button onClick={handleClose} className={`${styles.dilog_delete_no_btn}`} variant='outlined'>
                            No
                        </Button>
                        <Button onClick={() => deleteRecord(deleteInfo.issue_id)} className={`${styles.dilog_delete_yes_btn}`} variant='outlined'>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
